...
class Btn(types.InlineKeyboardButton):
    """InlineKeyboardButton with optional Telegram Bot API button style."""
...
def _safe_extract_archive(zf: Any, target: Path) -> None:
    """Extract archive members safely; reject path traversal."""
...
def _safe_extract_tar(tf: Any, target: Path) -> None:
    """Extract tar members without allowing path traversal."""
    target = target.resolve()
    ...
    try:
        if file_path.endswith('.zip'):
            with zipfile.ZipFile(file_path, 'r') as z:
                ...
                _safe_extract_archive(z, tmp)
        elif file_path.endswith(('.tar.gz', '.tgz', '.tar')):
            with tarfile.open(file_path, 'r:*') as t:
                _safe_extract_tar(t, Path(tmp))
...
# │  BOT TOKEN  add karo.   ││
# └──────────────────────────────────────────────────────────────┘
BOT_TOKEN_HARDCODED = ""  # Deprecated; intentionally unused.
...
# SECURITY: capture host-only secrets in process memory and immediately remove
# them from the OS environment. Child bots are untrusted code; keeping the
# host secrets in os.environ would make them discoverable through /proc on
# some Linux deployments. Child processes receive only their own env later.
_HOST_SECRET_ENV: Dict[str, str] = {}
for _secret_name in {
    "BOT_TOKEN", "OWNER_ID", "ERROR_BOT_TOKEN", "BHARATPE_TOKEN",
    "BHARATPE_MERCHANT_ID", "GITHUB_TOKEN", "GITHUB_REPO", "GITHUB_BRANCH",
    "GITHUB_KEY_REPO", "OPENAI_API_KEY", "ANTHROPIC_API_KEY", "GROQ_API_KEY",
    "AI_INTEGRATIONS_OPENROUTER_API_KEY", "SESSION_SECRET", "SECRET_KEY",
}:
    _v = os.environ.get(_secret_name)
    if _v is not None:
        _HOST_SECRET_ENV[_secret_name] = str(_v).strip()
...
SUPPORT_USR = "@FLEXSAMAY"
UPDATE_CH   = "https://t.me/KRYON_JI"
...
PAYMENT_METHODS: Dict[str, Dict[str, Any]] = {
    "phonepe":  {"name": "PhonePe",       "number": "Contact admin for UPI ID", "type": "UPI",                    "tag": "[PP]"},
    "paytm":    {"name": "Paytm",         "number": "Contact admin for UPI ID", "type": "UPI",                    "tag": "[P]"},
    "gpay":     {"name": "Google Pay",    "number": "Contact admin for UPI ID", "type": "UPI",                    "tag": "[GP]"},
    "fampay":   {"name": "FamPay",        "number": "Contact admin for UPI ID", "type": "UPI",                    "tag": "[FP]"},
    "navi":     {"name": "Navi UPI",      "number": "Contact admin for UPI ID", "type": "UPI",                    "tag": "[NU]"},
    "bharatpe": {"name": "BharatPe — Auto Verification", "number": "Contact admin for BharatPe UPI ID", "type": "UPI • Auto Verification", "tag": "[BH]", "auto_verify": True},
    "binance":  {"name": "Binance Pay",   "number": "Contact admin for Binance Pay ID", "type": "USDT (BEP20/TRC20)",    "tag": "[BP]"},
    "bank":     {"name": "Bank Transfer", "number": "Contact admin for bank details", "type": "Indian Bank Transfer", "tag": "[BK]"},
}

UPI_PAYMENT_KEYS = {"phonepe", "paytm", "gpay", "fampay", "navi", "bharatpe"}

def _payment_value(key: str) -> str:
    """Return the admin-configured payment address/details."""
    m = PAYMENT_METHODS.get(key, {})
    return str(get_setting(f"pm_number_{key}", m.get("number", "")) or m.get("number", ""))

def _is_valid_upi_id(value: str) -> bool:
    value = str(value or "").strip()
    return bool(re.fullmatch(r"[A-Za-z0-9._-]{2,128}@[A-Za-z0-9._-]{2,64}", value))

def _upi_qr_path(method: str, plan: Optional[str], amount: Any) -> Optional[str]:
    """Create/cache a standards-compatible UPI QR for a plan payment."""
    if method not in UPI_PAYMENT_KEYS:
        return None
    upi_id = _payment_value(method).strip()
    if not _is_valid_upi_id(upi_id):
        return None
    try:
        amt = float(amount or 0)
        if amt <= 0:
            return None
        amount_text = f"{amt:.2f}".rstrip("0").rstrip(".")
        name = PAYMENT_METHODS[method]["name"]
        plan_name = PLAN_LIMITS.get(plan or "", {}).get("name", "Hosting Plan")
        txn_note = f"{name} - {plan_name}"[:80]
        uri = (
            "upi://pay?"
            f"pa={quote(upi_id, safe='@')}"
            f"&pn={quote(str(get_setting('payment_merchant_name', BRAND)), safe='')}"
            f"&am={quote(amount_text, safe='')}"
            "&cu=INR"
            f"&tn={quote(txn_note, safe='')}"
        )
        qr_dir = DIRS["data"] / "payment_qr"
        qr_dir.mkdir(parents=True, exist_ok=True)
        digest = hashlib.sha256(uri.encode()).hexdigest()[:24]
        path = qr_dir / f"upi_{digest}.png"
...
def _payment_admin_label(method: str) -> str:
    if method == "bharatpe":
        return "BharatPe UPI ID"
    return "UPI ID" if method in UPI_PAYMENT_KEYS else ("Bank Details" if method == "bank" else "Address / ID")

SECRET_ENV_NAMES = {
    "BOT_TOKEN", "OWNER_ID", "ERROR_BOT_TOKEN", "BHARATPE_TOKEN", "BHARATPE_MERCHANT_ID",
    ...
    "AI_INTEGRATIONS_OPENROUTER_API_KEY", "SESSION_STRING", "SECRET_KEY",
}
...
_PHOTO_SPECS: Dict[str, Tuple[str, str, str]] = {
    ...
    "welcome":   ("Wᴇʟᴄᴏᴍᴇ",         "#0F172A", "𓂀KRYON_HOST𓂀"),
...
# ─────────────────────────────────────────────────────────────────────────────
# SECRET VAULT (at-rest protection for child bot ENV)
# ─────────────────────────────────────────────────────────────────────────────
# Child bot credentials must never be written as plaintext JSON.  The vault
# key is derived from the HOST BOT token and is never stored in the database
# or GitHub backup.  This is intentionally separate from the per-file Fernet
# keys used for uploaded source files.
_VAULT_PREFIX = "enc:v1:"

def _vault_fernet() -> Fernet:
    material = ("KRYON-HOST-ENV-VAULT-v1|" + TOKEN).encode("utf-8")
    digest = hashlib.sha256(material).digest()
    return Fernet(base64.urlsafe_b64encode(digest))

def _vault_encrypt(value: Any) -> str:
    raw = str(value if value is not None else "").encode("utf-8")
    return _VAULT_PREFIX + _vault_fernet().encrypt(raw).decode("ascii")

def _vault_decrypt(value: Any) -> str:
    text = "" if value is None else str(value)
    if not text.startswith(_VAULT_PREFIX):
        return text
    try:
        raw = _vault_fernet().decrypt(text[len(_VAULT_PREFIX):].encode("ascii"))
        return raw.decode("utf-8")
    except Exception:
        # Keep an unreadable value masked in memory rather than exposing or
        # guessing at a credential. The owner can replace it from Env Editor.
        return ""

def _decrypt_bot_envs_in_place(d: Dict[str, Any]) -> Dict[str, Any]:
    for b in (d.get("bots") or {}).values():
        env = b.get("env")
        if not isinstance(env, dict):
            b["env"] = {}
            continue
        for k, v in list(env.items()):
            env[str(k).upper()] = _vault_decrypt(v)
            if str(k).upper() != k:
                env.pop(k, None)
    return d

def _encrypt_db_copy(d: Dict[str, Any]) -> Dict[str, Any]:
    out = copy.deepcopy(d)
    for b in (out.get("bots") or {}).values():
        env = b.get("env")
        if isinstance(env, dict):
            ...
            for k, v in env.items():
                ...
                if sv.startswith(_VAULT_PREFIX):
                    protected_env[sk] = sv
                else:
                    protected_env[sk] = _vault_encrypt(sv)
            b["env"] = protected_env
    return out

def _backup_db_bytes() -> bytes:
    # Always build the backup from the in-memory/decrypted model and then
    # encrypt child ENV values before serializing. This also protects legacy
    # databases that pre-date the vault.
    protected = _encrypt_db_copy(db_load())
    return json.dumps(protected, ensure_ascii=False, indent=2).encode("utf-8")

def _is_secret_setting_key(key: str) -> bool:
    lk = str(key or "").lower()
    return any(w in lk for w in ("token", "password", "secret", "api_key", "private_key"))

def _decrypt_settings_in_place(d: Dict[str, Any]) -> Dict[str, Any]:
    for k, v in list(d.items()):
        if _is_secret_setting_key(k) and isinstance(v, str):
            d[k] = _vault_decrypt(v)
    return d

def _encrypt_settings_copy(d: Dict[str, Any]) -> Dict[str, Any]:
    out = copy.deepcopy(d)
    for k, v in list(out.items()):
        if _is_secret_setting_key(k) and isinstance(v, str):
            out[k] = v if v.startswith(_VAULT_PREFIX) else _vault_encrypt(v)
    return out

def _safe_settings_for_backup(settings: Dict[str, Any]) -> Dict[str, Any]:
    out = {}
    secret_words = ("token", "password", "secret", "api_key", "private_key")
    for k, v in (settings or {}).items():
        lk = str(k).lower()
        if any(w in lk for w in secret_words):
            continue
        out[k] = v
    return out
...
def _ensure_db_defaults(d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return _decrypt_bot_envs_in_place(d)
...
def db_save(d: Dict[str, Any]) -> None:
    with _db_lock:
        # Encrypt child ENV values on disk; keep the caller's in-memory copy
        # unchanged so runtime code can use normal strings.
        protected = _encrypt_db_copy(d)
        _atomic_write(DB_FILE, protected)
        _cache_invalidate(DB_FILE)


def settings_load() -> Dict[str, Any]:
    with _db_lock:
        d = _cached_load(SETTINGS_FILE, {})
    return _decrypt_settings_in_place(d)


def settings_load_ro() -> Dict[str, Any]:
    """Read-only fast path — DO NOT mutate."""
    with _db_lock:
        d = _cached_load_ro(SETTINGS_FILE, {})
    return _decrypt_settings_in_place(d)


def settings_save(d: Dict[str, Any]) -> None:
    with _db_lock:
        _atomic_write(SETTINGS_FILE, _encrypt_settings_copy(d))
        _cache_invalidate(SETTINGS_FILE)


def _secure_storage_migrate() -> None:
    """One-time migration for legacy plaintext secrets on local disk."""
    try:
        if DB_FILE.exists():
            d = db_load()
            db_save(d)  # encrypts every child ENV value on disk
        if SETTINGS_FILE.exists():
            d = settings_load()
            settings_save(d)  # encrypts secret setting values on disk
    except Exception as e:
        print(f"[security] secret-storage migration warning: {e}", file=sys.stderr)
...
def is_admin(uid: int) -> bool:
    # Additional admins are managed from the Admin Panel and persisted in the DB.
    return is_owner(uid) or str(uid) in db_load_ro().get("admins", {})
...
class KeyRing:
    ...
    @staticmethod
    def _gh_token() -> str:
        return (_HOST_SECRET_ENV.get("GITHUB_TOKEN") or get_setting("github_token", "") or "").strip()

    @staticmethod
    def _gh_key_repo() -> str:
        # Prefer a separate repo for keys; falls back to backup repo
        return (
            _HOST_SECRET_ENV.get("GITHUB_KEY_REPO")
            or get_setting("github_key_repo", "")
            or _HOST_SECRET_ENV.get("GITHUB_REPO")
    ...
    def _gh_request(self, method: str, path: str, **kw) -> Optional[requests.Response]:
        ...
        h.setdefault("User-Agent", "kryon-host/1.0")
        try:
    ...
    def store(self, key_id: str, key: bytes, meta: Dict[str, Any]) -> bool:
        ...
        # Never store raw file-encryption keys in GitHub. The backup repo may
        # be private, but repository access must not be enough to decrypt
        # uploaded source. The key itself is wrapped with the host-only vault.
        body = {"key": _vault_encrypt(key.decode()), "meta": meta, "ts": ts_iso()}
        ...
        if self.gh_enabled():
            ...
            if r is not None and r.status_code == 200:
                try:
                    ...
                    stored_key = blob["key"]
                    if isinstance(stored_key, str) and stored_key.startswith(_VAULT_PREFIX):
                        key = _vault_decrypt(stored_key).encode()
                        if not key:
                            raise ValueError("vault key unavailable")
                    else:
                        # Legacy keyring entries may contain plaintext keys;
                        # accept them for migration, but new writes are wrapped.
                        key = str(stored_key).encode()
                    with self._lock:
                        self._mem[key_id] = key
                    return key
                except Exception:
...
SECURE_EXEC = BASE_DIR / "secure_exec.py"
    ...
    if zip_files:
        ...
        try:
            with _zf.ZipFile(zip_files[0], "r") as z:
                _safe_extract_archive(z, bot_dir)
        except Exception:
            return (None, None)
        # recursive re-check
        py_files = _iter_user_files(bot_dir, ".py")
...
def _read_local_env_files(bot_dir: Path) -> Dict[str, str]:
    """Load simple .env files shipped with a child bot."""
    out: Dict[str, str] = {}
    for path in (bot_dir / ".env", bot_dir / ".env.local", bot_dir / "config.env"):
        if not path.is_file():
            continue
        try:
            for raw in path.read_text(encoding="utf-8", errors="ignore").splitlines():
                ...
                if len(value) >= 2 and value[0] == value[-1] and value[0] in {"'", '"'}:
                    value = value[1:-1]
                if key in {"BOT_TOKEN", "TELEGRAM_BOT_TOKEN", "MAIN_BOT_TOKEN"}:
                    value = "".join(value.split())
                elif key.endswith(("_URL", "_URI", "_ENDPOINT")):
                    value = value.replace("\r", "").replace("\n", "").strip()
                else:
                    value = value.strip()
                out[key] = value
        except Exception as e:
            print(f"[dotenv] skipped {path.name}: {e}", file=sys.stderr)
    return out


# ── Robust child ENV parsing/repair ──────────────────────────────
# Users often paste a complete .env block into the single KEY=VALUE
# prompt.  Older builds stored the whole block as BOT_TOKEN and then
# ``"".join(value.split())`` glued the next variables onto the token,
# producing InvalidToken/InvalidURL errors.  Keep multiline input safe
# and automatically repair already-corrupted saved env dictionaries.
_COMMON_ENV_KEYS = {
    "BOT_TOKEN", "TELEGRAM_BOT_TOKEN", "MAIN_BOT_TOKEN", "OWNER_ID", "OWNER_IDS",
    "LOGGER_ID", "LOG_CHANNEL", "LOG_CHANNEL_ID", "API_ID", "API_HASH",
    "MONGO_URI", "MONGODB_URI", "DATABASE_URL", "DATABASE_URI", "DB_URI",
    "GROQ_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY", "GOOGLE_API_KEY",
    "GITHUB_TOKEN", "GITHUB_REPO", "GITHUB_BRANCH", "ADMIN_ID", "ADMIN_IDS",
    "SUPPORT_CHAT", "SUPPORT_GROUP", "ANNOUNCE_CHANNEL", "UPDATE_CHANNEL",
    "PORT", "WEBHOOK_URL", "REDIS_URL", "REDIS_URI", "SESSION_STRING",
    "API_KEY", "SECRET_KEY", "WEB_URL", "BASE_URL", "CHAT_ID", "CHANNEL_ID",
}
...
def _clean_env_value(key: str, value: Any) -> str:
    ...
    # Remove matching wrapping quotes only; do not destroy internal chars.
    if len(v) >= 2 and v[0] == v[-1] and v[0] in {"'", '"'}:
        v = v[1:-1].strip()
    if k in {"BOT_TOKEN", "TELEGRAM_BOT_TOKEN", "MAIN_BOT_TOKEN"}:
        # Telegram tokens contain no whitespace.  Unlike the old code,
        # first remove only whitespace around the token, never concatenate
        # unrelated KEY=VALUE assignments into it.
        v = re.sub(r"\s+", "", v)
    elif k.endswith(("_URL", "_URI", "_ENDPOINT")):
...
def safe_env(bot_dir: Path, extra: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    ...
    # explicitly configured secrets and its local .env secrets.  The old
    # implementation removed BOT_TOKEN after loading the child environment,
    # which caused otherwise valid uploaded bots to die with:
    #   RuntimeError: BOT_TOKEN env variable missing
    env = {k: v for k, v in os.environ.items() if k not in SECRET_ENV_NAMES}

    # Project-local env files belong to the uploaded bot, so their values are
    # intentionally allowed (including BOT_TOKEN/API_HASH/etc.).
    local = _repair_env_dict(_read_local_env_files(bot_dir))
    child = _repair_env_dict(extra or {})

    # Child settings take precedence over project .env, and both take
    # precedence over the non-secret inherited host environment.
    env.update(local)
    env.update(child)

    # Normalize the three common Telegram token variable names one final time
    # without ever concatenating unrelated KEY=VALUE assignments.
    for key in ("BOT_TOKEN", "TELEGRAM_BOT_TOKEN", "MAIN_BOT_TOKEN"):
        if key in env:
            env[key] = _clean_env_value(key, env[key])
...
def _filter_third_party(modules: List[str], bot_dir: Path) -> List[str]:
    ...
    # local modules (recursively). Uploaded bots are often wrapped in a
    # subfolder, so `database.py`/`utils.py` may not be at bot_dir itself.
    # Treat every local Python module/package as project code rather than
    # trying to install a same-named PyPI package.
    deps_dir = bot_dir / ".deps"
    for pyfile in bot_dir.rglob("*.py"):
        if deps_dir in pyfile.parents:
            continue
        skip.add(pyfile.stem)
    for init in bot_dir.rglob("__init__.py"):
        if deps_dir in init.parents:
            continue
        skip.add(init.parent.name)
        ...
        if kind == "python":
            ...
            try:
                ...
                if third_party:
                    ...
                    # Install each scanned package independently. A missing/non-PyPI
                    # local import such as `database` must not block valid packages
                    # (for example Hydrogram) from being installed.
                    install_failures = 0
                    for package in third_party:
                        r2 = subprocess.run(
                            [sys.executable, "-m", "pip", "install",
                             "--target", str(deps_dir), *_PIP_BASE_FLAGS, package],
                            cwd=str(bot_dir), timeout=600, capture_output=True, text=True,
                            env=pip_env,
                        )
                        for line in (r2.stdout or "").splitlines()[-8:]:
                            log.append(line)
                        for line in (r2.stderr or "").splitlines()[-8:]:
                            log.append(line)
                        if r2.returncode != 0:
                            install_failures += 1
                            log.append(f"[{G['warn']}] skipped package {package} (rc={r2.returncode}); continuing")
                        else:
                            log.append(f"[{G['ok']}] installed {package}")
                    log.append(
                        f"[{G['ok']}] auto-install scan finished "
                        f"(failed={install_failures})"
                    )
            except Exception as e:
...
_TELEGRAM_TOKEN_REDACT_RE = re.compile(r"\b\d{8,10}:[A-Za-z0-9_-]{30,50}\b")


def _redact_runtime_log(text: str) -> str:
    s = str(text or "")
    s = _TELEGRAM_TOKEN_REDACT_RE.sub("[REDACTED_TELEGRAM_TOKEN]", s)
    # Redact common secret assignments without logging the actual value.
    s = re.sub(r"(?i)(\b(?:BOT_TOKEN|TELEGRAM_BOT_TOKEN|MAIN_BOT_TOKEN|API_KEY|API_HASH|PASSWORD|SECRET|MONGO_URI|MONGODB_URI|GROQ_API_KEY|OPENAI_API_KEY|BHARATPE_TOKEN)\s*[=:]\s*)[^\s]+", r"\1[REDACTED]", s)
    s = re.sub(r"(?i)(mongodb(?:\+srv)?://)[^\s]+", r"\1[REDACTED]", s)
    return s
...
def _drain_proc(bot_id: str, proc: subprocess.Popen, log: List[str]) -> None:
    try:
        ...
        for line in iter(proc.stdout.readline, b""):
            try:
                txt = _redact_runtime_log(line.decode("utf-8", "replace").rstrip())
            except Exception:
                txt = repr(line)
            log.append(txt)
            if len(log) > LOG_RING:
    ...
    try:
        ...
        if PLAN_LIMITS.get(plan, {}).get("auto_restart") and plan_active and not was_manual and not memory_limited:
            ...
            if len(history) >= 5:
                b_doc["_restart_history"] = history
                b_doc["status"] = "crashed"
                b_doc["last_error"] = (b_doc.get("last_error") or "") + "\nAuto-restart stopped after 5 crashes in 10 minutes."
                save_bot(b_doc)
                log.append(f"[{G['warn']}] auto-restart disabled: crash loop detected")
                return
            history.append(now)
            b_doc["_restart_history"] = history
            save_bot(b_doc)
...
def start_child(b: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if b.get("status") != "running":
        b.pop("_restart_history", None)
    install_deps(bot_dir, kind, log)
    # Run every untrusted child through secure_exec.py. It disables core
    # dumps/ptrace visibility (PR_SET_DUMPABLE=0 where supported) before
    # exec'ing the real bot, reducing cross-process credential exposure.
    if kind == "node":
        cmd = [sys.executable, str(SECURE_EXEC), "--", "node", entry]
    else:
        cmd = [sys.executable, str(SECURE_EXEC), "--", sys.executable, "-u", entry]

    # Merge/repair saved child ENV.  If BOT_TOKEN is absent from the saved
    # panel variables, the project's own .env files are still valid sources.
    extra_env = _repair_env_dict(dict(b.get("env") or {}))
    local_env = _repair_env_dict(_read_local_env_files(bot_dir))
    for _k, _v in local_env.items():
        extra_env.setdefault(_k, _v)
    if extra_env != (b.get("env") or {}):
        b["env"] = extra_env
        try:
            save_bot(b)
        except Exception:
            pass
    run_env = safe_env(bot_dir, extra_env)
    # Never log the secret itself; only record whether a child token exists.
    if not any(run_env.get(k) for k in ("BOT_TOKEN", "TELEGRAM_BOT_TOKEN", "MAIN_BOT_TOKEN")):
        log.append("[ENV] BOT_TOKEN not found in child panel ENV or local .env")
    # Keep both the project root and the entry file's directory importable.
    # This is important for ZIPs such as MyBot/main.py + database.py.
    entry_parent = (bot_dir / entry).parent
    pp_parts = [str(bot_dir), str(entry_parent), str(bot_dir / ".deps")]
    existing_pp = run_env.get("PYTHONPATH", "")
    if existing_pp:
        pp_parts.append(existing_pp)
    run_env["PYTHONPATH"] = os.pathsep.join(dict.fromkeys(pp_parts))
    try:
        proc = subprocess.Popen(
            cmd, cwd=str(bot_dir), env=run_env,
    ...
    # after startup; overwriting .py/.js files after 6 seconds caused silent
    # post-start failures and made /start appear unresponsive.
...
def tail_log(bot_id: str, lines: int = 60) -> str:
    """Return the most recent runtime log for a hosted bot.

    The old UI called an undefined tail_log(), which made Live Logs fail
    even when the child process itself was running. Prefer the live ring
    buffer and fall back to the persisted last error after a crash.
    """
    try:
        info = RUNNING.get(bot_id)
        if info:
            entries = list(info.get("log") or [])
            if entries:
                return "\n".join(entries[-max(1, int(lines)):])
        b = find_bot(bot_id)
        if b:
            err = str(b.get("last_error") or "").strip()
            if err:
                return err
    except Exception as e:
...
def gh_load_config() -> None:
    GH["token"]  = _HOST_SECRET_ENV.get("GITHUB_TOKEN")  or get_setting("github_token", "")  or ""
    GH["repo"]   = _HOST_SECRET_ENV.get("GITHUB_REPO")   or get_setting("github_repo", "")   or ""
    GH["branch"] = _HOST_SECRET_ENV.get("GITHUB_BRANCH") or get_setting("github_branch", "main") or "main"
    try:
...
def gh_sync_user_data() -> bool:
    ...
    try:
        ...
        if not DB_FILE.exists():
            return False
        # Never upload the raw database: legacy installs may still contain
        # plaintext child ENV. Re-serialize through the vault first.
        buf = _backup_db_bytes()
        ok = _gh_put_file("user_data.json", buf,
                          f"sync: user_data {ts_iso()}")
        # Settings can contain GitHub tokens from the optional panel config.
        # Back up only non-secret settings.
        if SETTINGS_FILE.exists():
            try:
                safe_settings = _safe_settings_for_backup(settings_load_ro())
                _gh_put_file("settings.json",
                             json.dumps(safe_settings, ensure_ascii=False, indent=2).encode("utf-8"),
...
def _gh_sync_bot_files(b: Dict[str, Any]) -> None:
    ...
    try:
        ...
        meta = json.dumps({
            ...
            "env_keys":  sorted((b.get("env") or {}).keys()),
            "cron":      b.get("cron", {}),
            "status":    b.get("status"),
...
def _normalize_chat_ref(value: Any) -> Any:
    """Convert public t.me URLs to Telegram @username chat references."""
    try:
        v = str(value or "").strip()
        if v.startswith("https://t.me/") or v.startswith("http://t.me/"):
            tail = v.split("t.me/", 1)[1].strip("/")
            # Invite links (+xxxx) are not valid send_message chat refs.
...
def post_announcement(html: str) -> None:
    if not ANNOUNCE_CHANNEL:
        return
    try:
        bot.send_message(_normalize_chat_ref(ANNOUNCE_CHANNEL), html, parse_mode="HTML")
    except Exception as e:
        print(f"[announce] {e}")
...
def save_bot(doc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        _atomic_write(bot_json, {
            ...
            "env":       {},  # secrets are stored only in encrypted panel DB
            "env_keys":  sorted((doc.get("env") or {}).keys()),
            "cron":      doc.get("cron", {}),
...
def ack(call: types.CallbackQuery, text: str = "") -> None:
    """Always acknowledge a callback; never let an ack failure hide routing."""
    try:
        bot.answer_callback_query(call.id, text=text or "")
    except Exception as e:
        print(f"[callback] answer failed: {e}", flush=True)
...
# No legacy third-party groups are required by the KRYON_HOST build.
# Add your own groups here only if you intentionally want join verification.
REQUIRED_GROUPS = []
...
# Legacy duplicate registration disabled.
...
def render_adm_analytics(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{G['div_eq']}\n"
        f"{bullet('Total Revenue',   money(total_rev))}\n"
...
def render_adm_revenue_report(call: types.CallbackQuery) -> None:
    ...
    by_plan = "\n".join(f"{bullet(k, money(v))}" for k, v in sorted(plan_rev.items()))
    cap = (
        f"<b>📈 {sc('Revenue Report')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Today',        money(rev_day))}\n"
        f"{bullet('Last 7 days',  money(rev_week))}\n"
        f"{bullet('Last 30 days', money(rev_month))}\n"
        f"{bullet('All time',     money(rev_all))}\n"
        f"{G['div']}\n<b>{sc('By Plan')}:</b>\n{by_plan}{FOOTER}"
    )
...
def render_adm_top_users(call: types.CallbackQuery) -> None:
    ...
    for i, (uid, amt) in enumerate(top, 1):
        ...
        rows.append(f"{i}. {name} — <b>{money(amt)}</b> {G['bullet']} {bot_count} bots")
    cap = (
...
_MESSAGE_TEMPLATES: Dict[str, Dict[str, str]] = {
    ...
    "payment_received": {
        ...
        "default": "✅ Payment of {amount}{currency_symbol} received for {plan} plan. Your account has been upgraded!",
    ...
    "referral_reward": {
        ...
        "default": "🎁 You earned {amount}{currency_symbol} for referring {referred_name}! Keep sharing!",
...
def _gh_api(endpoint: str, token: Optional[str] = None,
    ...
    req.add_header("User-Agent",    "KRYON_HOST/1.0")
    if body:
...
def render_adm_pay_config(call: types.CallbackQuery) -> None:
    ...
    merchant_name = get_setting("payment_merchant_name", BRAND) or BRAND
    cap = (
        ...
        f"{bullet('Merchant Name',  esc(merchant_name))}\n"
    ...
    kb.add(
        ...
        Btn("🏪  Mᴇʀᴄʜᴀɴᴛ Nᴀᴍᴇ", callback_data="adm_pay_merchant_name",      style="primary"),
    )
    kb.add(Btn("📋  Pᴀʏ Hɪꜱᴛᴏʀʏ", callback_data="adm_payments", style="primary"))
    kb.add(
...
def render_adm_pay_methods(call: types.CallbackQuery) -> None:
    ...
    for key, m in PAYMENT_METHODS.items():
        ...
        rows.append(f"{'✅' if enabled else '❌'} <b>{esc(m['name'])}</b> — "
                    f"<code>{esc(_payment_value(key))}</code> ({esc(m['type'])})")
    cap = (
...
def render_adm_pay_method_edit(call: types.CallbackQuery, key: str) -> None:
    ...
    cap = (
        ...
        f"{bullet(_payment_admin_label(key),  esc(stored_num))}\n"
...
def render_adm_pay_limits(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{G['div_eq']}\n"
        f"{bullet('Min Payment',      money(min_amt))}\n"
        f"{bullet('Max Payment',      money(max_amt))}\n"
        f"{bullet('Discount >= ' + currency_symbol(), disc_threshold)}\n"
...
def render_adm_pay_currency(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{bullet('Symbol',        sym)}\n"
        f"{G['div']}\n"
        f"{sc('Examples')}: INR/₹, USD/$, EUR/€, PKR/₨{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
    ...
    kb.add(Btn("INR ₹", callback_data="adm_bc_set_currency_INR_₹", style="primary"))
    kb.add(Btn(f"{G['back']}  Pᴀʏ Cᴏɴꜰɪɢ", callback_data="adm_pay_config", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("pay_config", PHOTOS["admin"]), cap, kb, call=call)
...
def action_adm_pay_method_number(call: types.CallbackQuery, data: str) -> None:
    ...
    if data.startswith("adm_pay_method_setnumber_"):
        ...
        bot.send_message(call.message.chat.id,
                         f"{G['settings']} {sc('Send new UPI ID / payment details for')} "
...
def render_adm_coupon_analytics(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{bullet('Total Savings Given', money(total_savings))}\n"
        f"{G['div']}\n<b>{sc('By Plan')}:</b>\n{plan_rows}\n{G['div']}{FOOTER}"
    )
    show_menu(call.message.chat.id, PHOTOS.get("coupon_plus", PHOTOS["coupon"]), cap,
              _adm_back("adm_coupon_plus"), call=call)
...
def render_adm_referral_sys(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{G['div_eq']}\n"
        f"{bullet('Status',        '✅ Enabled' if enabled else '❌ Disabled')}\n"
        f"{bullet('Reward/Refer',  f'{money(reward)} wallet credit')}\n"
        f"{bullet('Min Plan',      min_plan)}\n"
        f"{bullet('Total Referrals', total_refs)}\n"
        f"{bullet('Total Paid Out',  money(total_paid))}\n"
        f"{G['div']}{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
    ...
    kb.add(
        Btn(f"✏️  Sᴇᴛ Rᴇᴡᴀʀᴅ {currency_symbol()}", callback_data="adm_ref_set_reward",   style="primary"),
...
def render_adm_ref_stats(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        ...
        f"| earned {money(u.get('referral_earnings',0))}"
    ...
    cap = (
        ...
        f"{bullet('Total Paid',      money(total_paid))}\n"
...
def render_adm_ref_rewards(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{G['div_eq']}\n"
        f"{bullet('Base Reward', f'{money(reward)} per referral')}\n"
...
def render_adm_ref_leaderboard(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        ...
        f"{money(u.get('referral_earnings',0))} {sc('earned')}"
...
def action_adm_wh_test(call: types.CallbackQuery) -> None:
    ...
    def _bg() -> None:
        try:
            import urllib.request as _ur
            import json as _j
            payload = _j.dumps({"test": True, "ts": ts_iso(), "from": "KRYON_HOST"}).encode()
...
def render_adm_rev_goals(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"  {sc('Earned')}: <b>{money(rev_month)}</b> / {money(goal_month) if goal_month is not None else '—'}\n"
        f"  {progress_bar(rev_month, goal_month)}\n"
        f"{G['div']}\n"
        f"<b>{sc('This Year')} ({year_start})</b>\n"
        f"  {sc('Earned')}: <b>{money(rev_year)}</b> / {money(goal_year) if goal_year is not None else '—'}\n"
...
def render_adm_lb_referrals(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        f"{i}. <b>{esc(u.get('name','?')[:20])}</b> — "
        f"{len(u.get('referrals',[]))} refs | {money(u.get('referral_earnings',0))}"
...
def render_adm_bc_env_editor(call: types.CallbackQuery, bid: str) -> None:
    ...
    if not b:
        ack(call, "Bot not found"); return
    env = b.get("env", {})
    def _mask_env(k: str, v: Any) -> str:
        text = str(v or "")
        sensitive = k in SECRET_ENV_NAMES or any(x in k for x in ("TOKEN", "PASSWORD", "SECRET", "API_KEY", "API_HASH"))
        if sensitive:
            return (text[:4] + "…" + text[-4:]) if len(text) > 10 else "••••••"
        return text[:40]
    rows = "\n".join(
        f"{G['bullet']} <code>{esc(k)}</code> = <code>{esc(_mask_env(k, v))}</code>"
        for k, v in env.items()
...
def _do_export_data(admin_uid: int) -> Path:
    ...
    target = out / f"kryon_export_{stamp}.zip"
...
def _progress_bar(current, total=None, width=12):
    """Render a progress bar for either pct-only or current/total callers."""
    try:
        if total is None:
            pct = max(0.0, min(float(current) / 100.0, 1.0))
        elif float(total) <= 0:
            pct = 0.0
        else:
...
def action_adm_export_full_db(call):
    ...
    fname = f"kryon_db_{now_utc().strftime('%Y%m%d_%H%M%S')}.json"
    import io
...
def action_adm_export_users_csv(call):
    ...
    fname = f"kryon_users_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
...
def action_adm_export_bots_csv(call):
    ...
    fname = f"kryon_bots_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
...
def action_adm_export_trans_csv(call):
    ...
    fname = f"kryon_transactions_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
...
def action_adm_export_audit_csv(call):
    ...
    fname = f"kryon_audit_{now_utc().strftime('%Y%m%d')}.csv"
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
...
_CURRENCY_SYMBOLS = {
    ...
    "PHP":"₱","VND":"₫","PKR":"₨","INR":"₹","EGP":"£","NGN":"₦","KES":"Ksh",
}
_CRYPTO_SYMBOLS = {
...
def currency_code() -> str:
    code = str(get_setting("payment_currency", "INR") or "INR").upper().strip()
    return code if code in _CURRENCY_SYMBOLS else "INR"


def currency_symbol() -> str:
    sym = str(get_setting("currency_symbol", "₹") or "₹").strip()
    return sym or _CURRENCY_SYMBOLS.get(currency_code(), "₹")


def money(amount: Any) -> str:
    """Format all plan/wallet/payment amounts using the configured currency."""
    try:
        n = float(amount or 0)
        if n.is_integer():
            value = str(int(n))
        else:
            value = f"{n:.2f}".rstrip("0").rstrip(".")
    except Exception:
        value = str(amount)
    return f"{value}{currency_symbol()}"


def money_or_free(amount: Any) -> str:
    try:
    ...
    try:
        ...
        with target.open("rb") as fh:
            bot.send_document(
                ...
                parse_mode="HTML",
                visible_file_name=f"kryon_backup_{stamp}.zip",
            )
...
def render_plans_menu(call: types.CallbackQuery) -> None:
    ...
    for key, v in PLAN_LIMITS.items():
        price_txt = money_or_free(v["price"])
...
def render_plan_detail(call: types.CallbackQuery, plan: str) -> None:
    ...
    cap = (
        ...
        f"{bullet('Price', money_or_free(p['price']))}\n"
...
def render_payment_screen(call: types.CallbackQuery, data: str) -> None:
    ...
    if not pm or not bool(get_setting(f"pm_enabled_{method}", True)):
        ack(call, "Payment method unavailable")
        return

    p = PLAN_LIMITS.get(plan or "")
    amount = (p or {}).get("price", 0)
    configured = _payment_value(method)
    label = _payment_admin_label(method)

    cap = (
        f"<b>{pm['tag']} {esc(pm['name'])} — {sc('Payment')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet(label, esc(configured))}\n"
        f"{bullet('Type', esc(pm['type']))}\n"
    )
    if p:
        cap += f"{bullet('Plan', esc(p['name']))}\n{bullet('Amount', money(amount))}\n"

    qr_path = _upi_qr_path(method, plan, amount)
    if method in UPI_PAYMENT_KEYS:
        if qr_path:
            cap += f"{bullet('QR', 'Scan with any UPI app')}\n"
        else:
            cap += f"{G['no']} <b>{sc('UPI ID is not configured yet')}</b>\n"
        if method == "bharatpe":
            cap += f"{bullet('Verification', 'BharatPe Auto Verification')}\n"

    cap += (
        f"{G['div']}\n"
        f"<b>{sc('How to pay')}:</b>\n"
        f"1. {sc('Pay the exact amount shown above')}.\n"
        f"2. {sc('Tap Send Proof and send your receipt or transaction ID')}.\n"
        f"3. {sc('If auto-approval is enabled, a valid UTR can be verified automatically')}.\n"
        f"{G['div']}{FOOTER}"
    )

    USER_STATES[call.from_user.id] = {
        "flow": "await_payment_proof",
        "method": method,
        "plan": plan,
        "amount": amount,
        "payment_address": configured,
    }

    kb = types.InlineKeyboardMarkup()
    kb.add(Btn(f"{G['plus']}  {sc('Send Proof')}", callback_data="pay_proof"))
    kb.add(Btn(f"{G['back']}  {sc('Methods')}",
               callback_data=f"plan_buy_{plan}" if plan else "menu_buy"))

    # Send a real UPI QR when configured. If sending/editing fails, fall back
    # to the normal payment screen so the payment flow never gets stuck.
    if qr_path:
        try:
            if call.message and call.message.content_type == "photo":
                bot.edit_message_caption(
                    _html_safe_truncate(cap, 1024),
                    chat_id=call.message.chat.id,
                ...
                bot.send_photo(
                    call.message.chat.id,
                    types.InputFile(qr_path),
                    caption=f"<b>{esc(pm['name'])} — UPI QR</b>\n{bullet('Amount', money(amount))}",
                    parse_mode="HTML",
                )
                return
            bot.send_photo(
                call.message.chat.id,
                types.InputFile(qr_path),
                caption=_html_safe_truncate(cap, 1024),
                parse_mode="HTML",
                reply_markup=kb,
            )
            return
        except Exception as e:
            _log_err("send_payment_qr", e)

    show_menu(
        call.message.chat.id,
        PHOTOS.get("pay", PHOTOS["wallet"]),
        cap,
        kb,
        call=call,
    )
...
def render_wallet(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{G['div_eq']}\n"
        f"{bullet('Balance', money(u.get('wallet', 0)))}\n"
        f"{G['div']}\n"
...
def render_admin(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{bullet('Bots',    len(d['bots']))}\n"
        f"{bullet('Running', running_n)}\n"
        f"{bullet('Revenue', money(revenue))}\n"
...
def render_admin_subroute(call: types.CallbackQuery, data: str) -> None:
    ...
    # Comprehensive fallback for admin callbacks.  The project contains many
    # admin screens whose callback name directly matches their render function;
    # route those automatically instead of silently doing nothing.
    _direct_admin_fn = {
        "adm_pay_auto_approve": None,
        "adm_pay_merchant_name": None,
        "adm_pay_method_number": None,
        "adm_pay_method_toggle": None,
        "adm_pay_method_setnumber": None,
        "adm_tmpl_edit_payment_received": None,
        "adm_tmpl_reset_payment_received": None,
        "adm_bc_toggle_notif_on_new_payment": None,
        "adm_bc_toggle_notif_on_approved": None,
        "adm_bc_toggle_notif_on_rejected": None,
        "adm_bc_set_payment_notif_channel": None,
    }
    if data == "adm_pay_auto_approve":
        cur = bool(get_setting("auto_approve_payments", False))
        set_setting("auto_approve_payments", not cur)
        audit(uid, "payment_auto_approve_toggle", f"now={not cur}")
        ack(call, f"Auto-Approve: {'ON' if not cur else 'OFF'}")
        render_adm_pay_config(call); return
    if data.startswith("adm_pay_method_toggle_") or data.startswith("adm_pay_method_setnumber_"):
        action_adm_pay_method_number(call, data); return
    if data == "adm_pay_merchant_name":
        USER_STATES[uid] = {"flow": "await_adm_payment_setting", "setting_key": "payment_merchant_name", "kind": "text"}
        bot.send_message(call.message.chat.id, f"{G['settings']} Send the new payment merchant/display name:", parse_mode="HTML"); return
    if data.startswith("adm_bc_set_"):
        raw = data[len("adm_bc_set_"):]
        # Inputs are handled by one safe generic state handler below.
        mapping = {
            "min_payment_amount": ("min_payment_amount", "number"),
            "max_payment_amount": ("max_payment_amount", "number"),
            "discount_threshold": ("discount_threshold", "number"),
            "discount_pct": ("discount_pct", "number"),
            "payment_tax_pct": ("payment_tax_pct", "number"),
            "payment_currency": ("payment_currency", "currency"),
            "currency_symbol": ("currency_symbol", "text"),
            "payment_notif_channel": ("payment_notif_channel", "text"),
        }
        if raw in mapping:
            key, kind = mapping[raw]
            USER_STATES[uid] = {"flow": "await_adm_payment_setting", "setting_key": key, "kind": kind}
            bot.send_message(call.message.chat.id, f"{G['settings']} Send new value for <b>{esc(raw)}</b>:", parse_mode="HTML"); return
    if data == "adm_bc_set_currency_INR_₹":
        set_setting("payment_currency", "INR"); set_setting("currency_symbol", "₹")
        audit(uid, "currency_set", "INR/₹"); ack(call, "Currency set to INR ₹")
        render_adm_pay_currency(call); return
    if data == "adm_bc_toggle_notif_on_new_payment":
        k="notif_on_new_payment"; v=bool(get_setting(k, True)); set_setting(k, not v); ack(call, f"New payment notifications: {'ON' if not v else 'OFF'}"); render_adm_pay_notif_settings(call); return
    if data == "adm_bc_toggle_notif_on_approved":
        k="notif_on_approved"; v=bool(get_setting(k, True)); set_setting(k, not v); ack(call, f"Approved notifications: {'ON' if not v else 'OFF'}"); render_adm_pay_notif_settings(call); return
    if data == "adm_bc_toggle_notif_on_rejected":
        k="notif_on_rejected"; v=bool(get_setting(k, True)); set_setting(k, not v); ack(call, f"Rejected notifications: {'ON' if not v else 'OFF'}"); render_adm_pay_notif_settings(call); return
    if data == "adm_tmpl_edit_payment_received":
        USER_STATES[uid] = {"flow": "await_adm_payment_template", "template_key": "payment_received"}
        bot.send_message(call.message.chat.id, f"{G['settings']} Send the new payment receipt template. Variables: <code>{{name}}</code>, <code>{{amount}}</code>, <code>{{plan}}</code>, <code>{{tx_id}}</code>, <code>{{date}}</code>", parse_mode="HTML"); return
    if data == "adm_tmpl_reset_payment_received":
        set_setting("tmpl_payment_received", "")
        ack(call, "Payment receipt template reset")
        render_adm_pay_receipt_tmpl(call); return
    # Dynamic admin callbacks (buttons created with runtime keys/IDs)
    # These routes are intentionally handled before the generic name lookup.
    if data.startswith("adm_pay_edit_"):
        return render_adm_pay_method_edit(call, data[len("adm_pay_edit_"):])
    if data.startswith("adm_app_theme_"):
        key = data[len("adm_app_theme_"):]
        if key in _APPEARANCE_THEMES:
            set_setting("ui_theme", key); ack(call, f"Theme: {key}"); return render_adm_app_theme(call)
        ack(call, "Unknown theme"); return
    if data.startswith("adm_app_emoji_set_"):
        key = data[len("adm_app_emoji_set_"):]
        USER_STATES[uid] = {"flow":"await_adm_emoji", "emoji_key":key}
        bot.send_message(call.message.chat.id, f"✏️ Send emoji for <code>{esc(key)}</code> (send <code>-</code> to reset).", parse_mode="HTML"); return
    if data == "adm_app_emoji_reset":
        set_setting("custom_emojis", {}); ack(call, "Custom emojis reset"); return render_adm_app_emojis(call)
    if data.startswith("adm_tmpl_edit_"):
        key = data[len("adm_tmpl_edit_"):]
        if key in _MESSAGE_TEMPLATES:
            USER_STATES[uid] = {"flow":"await_adm_template", "template_key":key}
            bot.send_message(call.message.chat.id, f"📝 Send the new template for <b>{esc(_MESSAGE_TEMPLATES[key]['label'])}</b>.", parse_mode="HTML"); return
        ack(call, "Unknown template"); return
    if data.startswith("adm_jan_toggle_"):
        key = data[len("adm_jan_toggle_"):]
        set_setting(f"jan_{key}", not bool(get_setting(f"jan_{key}", False))); ack(call, "Janitor rule updated"); return render_adm_janitor(call)
    if data.startswith("adm_ff_toggle_"):
        key = data[len("adm_ff_toggle_"):]
        if key in _FEATURE_FLAG_DEFAULTS:
            _ff_set(key, not _ff_get(key)); ack(call, "Feature flag updated"); return render_adm_feature_flags(call)
        ack(call, "Unknown feature"); return
    if data.startswith("adm_rate_plan_"):
        return render_adm_rate_plan(call, data[len("adm_rate_plan_"):])
    if data.startswith("adm_rate_set_"):
        raw = data[len("adm_rate_set_"):]
        parts = raw.split("_", 1)
        if len(parts) == 2 and parts[0] in _RATE_LIMIT_DEFAULTS and parts[1] in ("uploads_per_day","starts_per_hour","msgs_per_min"):
            USER_STATES[uid] = {"flow":"await_adm_rate", "plan":parts[0], "metric":parts[1]}
            bot.send_message(call.message.chat.id, f"⏱️ Send new value for <code>{esc(parts[0])} / {esc(parts[1])}</code>:", parse_mode="HTML"); return
        ack(call, "Invalid rate setting"); return
    if data.startswith("adm_sched_toggle_"):
        tid = data[len("adm_sched_toggle_"):]; val = _sched_toggle_task(tid); ack(call, "Task enabled" if val else "Task disabled" if val is False else "Task not found"); return render_adm_sched_list(call)
    if data.startswith("adm_sched_del_"):
        tid = data[len("adm_sched_del_"):]; ok = _sched_remove_task(tid); ack(call, "Task deleted" if ok else "Task not found"); return render_adm_sched_list(call)
    if data.startswith("adm_lang_set_"):
        code = data[len("adm_lang_set_"):]
        if code in _SUPPORTED_LANGUAGES:
            set_setting("default_language", code); ack(call, f"Language: {_SUPPORTED_LANGUAGES[code]}"); return render_adm_languages(call)
        ack(call, "Unknown language"); return
    if data.startswith("adm_bcbot_"):
        prefix = data[len("adm_bcbot_"):]
        b = next((x for x in db_load()["bots"].values() if str(x.get("_id",""))[:20] == prefix), None)
        if b: return render_adm_bc_single(call, b["_id"])
        ack(call, "Bot not found"); return
    if data.startswith("adm_bc_stop_") or data.startswith("adm_bc_restart_"):
        prefix = data.split("_", 3)[3]
        b = next((x for x in db_load()["bots"].values() if str(x.get("_id",""))[:20] == prefix), None)
        if not b: ack(call, "Bot not found"); return
        try:
            if data.startswith("adm_bc_stop_"): stop_child(b["_id"], manual=True); ack(call, "Stopped")
            else: start_child(b["_id"]); ack(call, "Started")
        except Exception as e: ack(call, f"Failed: {e}")
        return render_adm_bc_single(call, b["_id"])
    if data.startswith("adm_bc_logs_") or data.startswith("adm_bc_env_") or data.startswith("adm_bc_res_") or data.startswith("adm_bc_del_"):
        prefix = data.split("_", 3)[3]
        b = next((x for x in db_load()["bots"].values() if str(x.get("_id",""))[:20] == prefix), None)
        if not b: ack(call, "Bot not found"); return
        bid = b["_id"]
        if data.startswith("adm_bc_logs_"): return render_adm_bc_logs(call, bid)
        if data.startswith("adm_bc_env_"): return render_adm_bc_env_editor(call, bid)
        if data.startswith("adm_bc_res_"): return render_adm_bc_resources(call, bid)
        return render_bot_delete_confirm(call, bid)
    ...
    if data.startswith("adm_ghfile_"):
        try: idx = int(data[len("adm_ghfile_"):])
        except Exception: ack(call, "Invalid file"); return
        state = get_setting("github_browser_files", []) or []
        if 0 <= idx < len(state): return render_adm_gh_file_view(call, state[idx].get("repo", GH.get("repo","")), state[idx].get("path", ""))
        ack(call, "File not found"); return
    if data.startswith("adm_photo_"):
        return render_adm_photo_one(call, data[len("adm_photo_"):])

    # Generic exact-name routing covers the large advanced admin surface.
    _candidate = "render_" + data
    _fn = globals().get(_candidate)
    if callable(_fn):
        _fn(call); return
    _candidate = "render_adm_" + data[4:] if data.startswith("adm_") else ""
    _fn = globals().get(_candidate)
    if callable(_fn):
        _fn(call); return
...
def render_github_subroute(call: types.CallbackQuery, data: str) -> None:
    ...
    # Generic fallback for static admin screens.  A large part of the panel
    # follows the convention callback_data="adm_<name>" ->
    # render_adm_<name>(call).  Older revisions had buttons for these screens
    # without adding every single entry to the dispatcher, so the button
    # appeared to do nothing.  Keep explicit/dynamic routes above, then safely
    # resolve any matching renderer here.
    if data.startswith("adm_"):
        fn = globals().get("render_" + data)
        if callable(fn):
            try:
                fn(call)
            except Exception as e:
                _log_err(f"admin_callback:{data}", e)
                ack(call, "Unable to open this section")
            return
...
def _handle_env_kv(m: types.Message, st: Dict[str, Any]) -> None:
    ...
    if "\n" in text or "\r" in text:
        parsed = _repair_env_dict({"__PASTED_ENV__": text})
    else:
        k, v = text.split("=", 1)
        k = k.strip().upper()
        if not re.match(r"^[A-Za-z_][A-Za-z0-9_]*$", k):
            bot.reply_to(m, f"{G['no']} invalid env key"); return
...
# ─────────────────────────────────────────────────────────────────────────────
# BharatPe UTR auto-verification (optional, env-configured)
# ─────────────────────────────────────────────────────────────────────────────

def _extract_utr(text: str) -> Optional[str]:
    text = str(text or "").strip()
    m = re.search(r"(?:\bUTR\b|\bRRN\b|\bTransaction(?:\s+ID)?\b)\s*[:#=\-]?\s*([A-Za-z0-9]{8,30})",
                  text, re.I)
    if m:
        return m.group(1).strip()
    # Common UPI UTR/RRN values are 10–22 digits. Only treat a bare numeric
    # message as a UTR when it is long enough to avoid accidental matches.
    if re.fullmatch(r"\d{10,22}", text):
        return text
    return None


def _verify_bharatpe_payment(utr: str, amount: float) -> str:
    """Return SUCCESS, DUPLICATE, NOT_FOUND, or API_ERROR."""
    utr = str(utr or "").strip()
    if not utr:
        return "NOT_FOUND"

    # Prevent the same UTR from being approved twice.
    try:
        d = db_load_ro()
        for payment in d.get("payments", []):
            if str(payment.get("utr", "")).strip() == utr:
                return "DUPLICATE"
    except Exception:
        pass

    merchant_id = _HOST_SECRET_ENV.get("BHARATPE_MERCHANT_ID", "").strip()
    token = _HOST_SECRET_ENV.get("BHARATPE_TOKEN", "").strip()
    if not merchant_id or not token:
        return "API_ERROR"

    url = "https://payments-tesseract.bharatpe.in/api/v1/merchant/transactions"
    params = {"module": "PAYMENT_QR", "merchantId": merchant_id}
    ...
    try:
        ...
        for tx in transactions:
            tx_utr = (
                tx.get("utr")
                or tx.get("bankReferenceNo")
                or tx.get("bankTxnId")
                or tx.get("transactionId")
                or tx.get("id")
            )
            if not tx_utr or str(tx_utr).strip() != utr:
                continue
            try:
                tx_amount = float(tx.get("amount"))
            except (TypeError, ValueError):
                continue
            if abs(tx_amount - float(amount)) >= 0.01:
                continue
            status = str(tx.get("status", "")).upper()
            if status in {"SUCCESS", "SUCCESSFUL", "COMPLETED", "ACTIVE", "PAID"}:
                return "SUCCESS"
        return "NOT_FOUND"
    except Exception as e:
        _log_err("bharatpe_verify", e)
        return "API_ERROR"
...
def _handle_payment_proof(m: types.Message, st: Dict[str, Any]) -> None:
    ...
    plan = st.get("plan")
    p = PLAN_LIMITS.get(plan or "")
    amount = float(st.get("amount", (p or {}).get("price", 0)) or 0)
    ...
    payment = {
        ...
        "amount": amount, "status": "pending", "ts": ts_iso(),
        "telegram_msg_id": m.message_id,
    }
    if utr:
        payment["utr"] = utr
    d["payments"].append(payment)
    db_save(d)
    USER_STATES.pop(m.from_user.id, None)

    auto_enabled = bool(get_setting("auto_approve_payments", False))
    verification = "SKIPPED"
    if auto_enabled and utr and method in UPI_PAYMENT_KEYS and amount > 0:
        verification = _verify_bharatpe_payment(utr, amount)
        if verification == "SUCCESS" and _auto_approve_payment(pid, OWNER_ID):
            bot.reply_to(
                m,
                f"<b>{G['ok']} Payment verified & approved automatically.</b>\n"
                f"{bullet('UTR', esc(utr))}\n{bullet('Amount', money(amount))}",
                parse_mode="HTML",
            )
            return
    ...
    notify_owner(
        ...
        f"{bullet('Amount', money(amount))}\n{bullet('UTR', utr or '—')}\n"
        f"{bullet('Auto check', verification)}"
    )
    try:
        bot.send_message(OWNER_ID, f"<b>Decide #{pid}</b>",
                         parse_mode="HTML", reply_markup=kb)
    except Exception:
        pass
    ...
    bot.reply_to(m, f"<b>{G['ok']} Proof received</b> #{pid} — await admin.",
                 parse_mode="HTML")
...
def on_text(m: types.Message) -> None:
    ...
    flow = st.get("flow", "")
    try:
        if flow == "await_adm_payment_setting":
            if not is_owner(uid): USER_STATES.pop(uid, None); return
            key = str(st.get("setting_key", "")); kind = str(st.get("kind", "text"))
            value = text
            try:
                if kind == "number":
                    value = float(text)
                    if value < 0: raise ValueError("value must be non-negative")
                    if float(value).is_integer(): value = int(value)
                elif kind == "currency":
                    value = text.upper().strip()
                    if value not in _CURRENCY_SYMBOLS: raise ValueError("unsupported currency code")
                elif not value and key != "payment_notif_channel":
                    raise ValueError("empty value")
                set_setting(key, value)
                audit(uid, "payment_setting", f"{key}={value}")
                USER_STATES.pop(uid, None)
                bot.reply_to(m, f"{G['ok']} {esc(key)} updated.", parse_mode="HTML")
            except Exception as e:
                bot.reply_to(m, f"{G['no']} Invalid value: <code>{esc(e)}</code>", parse_mode="HTML")
            return
        if flow == "await_adm_payment_template":
            if not is_owner(uid): USER_STATES.pop(uid, None); return
            set_setting("tmpl_payment_received", text[:1000])
            audit(uid, "payment_template_set", "payment_received")
            USER_STATES.pop(uid, None)
            bot.reply_to(m, f"{G['ok']} Payment receipt template updated.", parse_mode="HTML"); return
        ...
        if flow == "await_adm_pay_number":
            ...
            key = str(st.get("pm_key", ""))
            if key not in PAYMENT_METHODS:
                USER_STATES.pop(uid, None); bot.reply_to(m, f"{G['no']} Unknown payment method."); return
            val = text.strip()
            if PAYMENT_METHODS[key].get("type") == "UPI" and not _is_valid_upi_id(val):
                bot.reply_to(m, f"{G['no']} Invalid UPI ID. Example: <code>name@upi</code>", parse_mode="HTML"); return
            set_setting(f"pm_number_{key}", val)
            audit(uid, f"pm_set_{key}", "payment details updated")
            USER_STATES.pop(uid, None)
            bot.reply_to(m, f"{G['ok']} {esc(PAYMENT_METHODS[key]['name'])} details updated.", parse_mode="HTML"); return
        if flow == "await_adm_emoji":
            if not is_admin(uid): USER_STATES.pop(uid, None); return
            key = str(st.get("emoji_key", "")).strip()
            val = (text or "").strip()
            custom = get_setting("custom_emojis", {}) or {}
            if val == "-": custom.pop(key, None)
            elif key and len(val) <= 16: custom[key] = val
            else: bot.reply_to(m, f"{G['no']} Invalid emoji value"); return
            set_setting("custom_emojis", custom); USER_STATES.pop(uid, None); bot.reply_to(m, f"{G['ok']} Emoji updated."); return
        if flow == "await_adm_template":
            if not is_admin(uid): USER_STATES.pop(uid, None); return
            key = str(st.get("template_key", ""))
            if key not in _MESSAGE_TEMPLATES: USER_STATES.pop(uid, None); return
            set_setting(f"tmpl_{key}", text.strip()); USER_STATES.pop(uid, None); bot.reply_to(m, f"{G['ok']} Template updated."); return
        if flow == "await_adm_rate":
            if not is_admin(uid): USER_STATES.pop(uid, None); return
            plan, metric = str(st.get("plan","")), str(st.get("metric",""))
            try: val = max(0, int(text.strip()))
            except Exception: bot.reply_to(m, f"{G['no']} Send a whole number."); return
            if plan not in _RATE_LIMIT_DEFAULTS or metric not in ("uploads_per_day","starts_per_hour","msgs_per_min"):
                USER_STATES.pop(uid, None); bot.reply_to(m, f"{G['no']} Invalid rate setting."); return
            set_setting(f"rl_{plan}_{metric}", val); USER_STATES.pop(uid, None); bot.reply_to(m, f"{G['ok']} Rate limit updated."); return
        ...
        if flow == "await_adm_wallet_adjust":
            ...
            try:
                ...
                USER_STATES.pop(uid, None)
                bot.reply_to(m, f"{G['ok']} uid <code>{target_uid}</code>: <b>{money(cur)}</b> \u2192 <b>{money(new_bal)}</b>",
                             parse_mode="HTML")
        ...
        if flow == "await_adm_quick_announce":
            ...
            try:
                sent = bot.send_message(_normalize_chat_ref(ANNOUNCE_CHANNEL),
                    f"\U0001f4e3 <b>{BRAND_TAG}</b>\n{G['div']}\n{esc(msg_text)}", parse_mode="HTML")
                try: bot.pin_chat_message(_normalize_chat_ref(ANNOUNCE_CHANNEL), sent.message_id)
                except Exception: pass
...
def main() -> int:
    ...
    try:
        set_setting("payment_currency", "INR")
    ...
    try:
        ...
        pass
    # Migrate any legacy plaintext child ENV / secret settings immediately
    # after restore so they do not remain on disk in clear text.
    _secure_storage_migrate()
    # Background threads
